            ...
            async with engine.begin() as conn:
                if _is_sqlite:
                    await conn.execute(text(
                        "ALTER TABLE predictions ADD COLUMN model_insights TEXT"
                    ))
                else:
                    await conn.execute(text(
                        "ALTER TABLE predictions ADD COLUMN IF NOT EXISTS model_insights JSONB"
                    ))
            print("   ✅ model_insights column added")
        except Exception:
