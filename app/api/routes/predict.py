    ...
    try:
        ...
        if best_bet.get("has_edge") and best_bet.get("best_side") and best_bet.get("odds", 0) > 0:
            try:
                ...
                logger.info(f"CLV entry recorded: match={db_match.id}, side={best_bet['best_side']}")
            except Exception as e:
